...
export function KnowledgePanel({vaultPath,category}:{vaultPath:string;category:string}){
 ...
 return <div className="limen-card" style={{padding:24}}><h3>{labelIt(category)}</h3><p><code>{folders[category]}</code></p><p>Legge direttamente i file Markdown locali. Non richiede indice di ricerca né connessione di rete.</p><button disabled={busy} onClick={()=>setRefresh(n=>n+1)}>AGGIORNA CONOSCENZA</button>{busy?<p role="status">Caricamento delle note locali...</p>:error?<p role="alert">{<MessageIt value={error}/>}</p>:notes.length===0?<p>Nessuna nota Markdown in questa categoria.</p>:notes.map(n=><details key={n.relativePath}><summary>{n.title} · {labelIt(n.status)} · {n.relativePath}</summary><p style={{overflowWrap:'anywhere'}}>SHA-256 {n.sha256}</p><pre style={{whiteSpace:'pre-wrap'}}>{n.markdown}</pre></details>)}</div>
}
