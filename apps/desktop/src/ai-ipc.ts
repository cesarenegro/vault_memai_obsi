...
async function call<T>(command:string,args?:Record<string,unknown>):Promise<T>{if(!('__TAURI_INTERNALS__' in window))throw new Error('Native app required');return invoke<T>(command,args);}
export const aiIpc={
