// E:\Projects\vault_memai_obsi\apps\desktop\src-tauri\src\bin\diagnose_fase3b2.rs
// Diagnostica comparativa offline per il Passo 3b-2 (Punti C & D).
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("================================================================================");
    println!("DIAGNOSTICA COMPARATIVA PASSO 3b-2 — PUNTI C & D");
    println!("Vault: {:?}", vault_dir);
    println!("================================================================================");

    // Controlla porte standard o porta bge-m3 attiva (es. 59722, 62021)
    let candidate_ports = [59722, 62021, 8080];
    let mut active_port: Option<u16> = None;
    for &p in &candidate_ports {
        if llama::check_health(p) {
            println!("Servizio bge-m3 rilevato attivo sulla porta {}", p);
            active_port = Some(p);
            break;
        }
    }
    ...
    if active_port.is_none() {
        println!("Nessun server bge-m3 rilevato sulle porte di prova; select_with_port_timed eseguirà ricerca ibrida con fallback.");
    }
    ...
    for (idx, q) in queries.iter().enumerate() {
        ...
        match ai::select_with_port_timed(vault_dir, &options, active_port).await {
            Ok((sources, timings)) => {
                ...
                println!(
                    "Tempo anteprima (preview): {} ms (search: {} ms, passage_extract: {} ms)",
                    timings.t_preview_total_ms, timings.t_search_ms, timings.t_passage_extract_ms
                );
    ...
    println!("\n================================================================================");
    println!("DIAGNOSTICA COMPLETATA CON SUCCESSO");
    println!("================================================================================\n");
    Ok(())
}
