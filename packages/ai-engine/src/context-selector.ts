...
export function readDocument(vault: string, id: string, expectedHash: string, includeDrafts=false): SearchDocumentSource {
  ...
  if(!d||!allowedSource(d.relative_path)||(isAutomationPath(d.relative_path)&&!isCurrentAutomationDocument(vault,d.relative_path,d.sha256))||(!includeDrafts&&(d.status!=='approved'||['proposal','ai_output'].includes(d.category))&&!isCurrentAutomationDocument(vault,d.relative_path,d.sha256))) throw new Error('Document access denied');
...
export class VaultSearchProvider implements LocalSearchProviderContract {
 async queryDocuments(vault:string, term?:string, category?:string, tags?:string[], options?:AIQueryOptions){
  const rows=await new LocalSearchEngine().search(vault,{term,category:category as never,tags,client:options?.clientFilter,project:options?.projectFilter,limit:50});
  return rows.filter(r=>allowedSource(r.relativePath)&&(!options?.sourceIds?.length||options.sourceIds.includes(r.id))&& (options?.includeRawAndDrafts||r.status==='approved'&&!['proposal','ai_output'].includes(r.category)||isCurrentAutomationDocument(vault,r.relativePath,r.sha256))).slice(0,10).map(r=>readDocument(vault,r.id,r.sha256,options?.includeRawAndDrafts));
 }
}
...
export class ContextSelector {
 ...
 async selectContext(vault:string,o:AIQueryOptions,budget=16000):Promise<ContextSelectionResult>{
  ...
  for(const s of await this.searchProvider.queryDocuments(vault,o.prompt,o.categoryFilter,o.tagsFilter,o)){
   ...
   if(!o.includeRawAndDrafts&&(s.status!=='approved'||['proposal','ai_output'].includes(s.category))&&!isCurrentAutomationDocument(vault,s.relativePath,s.sha256))continue;
