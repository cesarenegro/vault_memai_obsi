# Stato storico precedente al piano completo per AG

18 settembre 2026. Snapshot documentale, non checklist operativa corrente. Per il nuovo incarico usare TASK_LIST.md e IMPL_PLANS/IMP_PLAN_MEMORIA_AZIENDALE_UI_UX.MD. Le verifiche e consegne precedenti non certificano il nuovo checkout.

## TASK_LIST precedente

# TASK LIST — LIMEN Vault: audit UI/UX completato, semplificazione e affidabilità da implementare

## Memoria aziendale semplice e consultabile — 18 settembre 2026

Stato: analisi sul codice reale completata; redesign non ancora implementato o consegnato. Le correzioni in corso restano locali e confluiscono nella consegna unica, senza pubblicare i candidati intermedi. Rapporto: `IMPLEMENTATION/2026-09-18_UX_MEMORIA_AZIENDALE.md`.

- [x] Audit di navigazione, pipeline manuale/automatica, inventario, lettore, retrieval, citazioni, indice, copie ed errori.
- [x] Inventario reale in sola lettura: 56 RAW; automazione non configurata; indice 58 documenti (57 proposte, una nota approvata), una nota in Casi studio.
- [x] Definita direzione UI: Chiedi, Documenti, Memoria; caricamento lime globale; strumenti tecnici secondari.
- [x] Definiti criteri di accettazione: originali apribili, testo ricercabile prima della wiki, risultati/citazioni apribili, stato unico, prove di ricerca e recupero.
- [ ] Unificare documento/revisioni/coda e separare estrazione/indice locale dall’API.
- [ ] Completare lettore, ricerca affidabile e retrieval per passaggi con valutazione su corpus rappresentativo.
- [ ] Implementare UI semplificata e migrazione conservativa del percorso precedente.
- [ ] Test end-to-end, aggiornamento HELP e consegna unica app/DMG notarizzati.


## Correzioni uso reale — 18 settembre 2026

- [x] Diagnosi verificata: 56 file RAW, configurazione automazione assente; 0/0 contava solo la coda, non gli originali.
- [x] Contatore basato sugli originali presenti, stato distinto pronto/in attesa/errore e messaggio di configurazione.
- [ ] Selezione modelli disponibili, dettaglio risultati ricerca e parole evidenziate. Selettore account in sorgente locale, ancora da completare per compatibilità e collaudo; apertura/evidenziazione non implementate.
- [ ] Chiarimenti UI parzialmente introdotti; guida e flusso finale da riallineare al redesign. Typecheck desktop PASS; non equivale al collaudo finale.
- [ ] Test, prova UI, build unica finale notarizzata e consegna con nome v3 e pulsante lime.

## Pulsante CARICA DOCUMENTI — 18 settembre 2026

- [x] Applicati sfondo verde lime, testo nero, grassetto, dimensioni maggiori, focus tastiera visibile e stato di caricamento.
- [ ] Verifiche finali, resa UI, build e pacchetto notarizzato con entrambe le correzioni.
- [ ] Aggiornamento USER INSTALL e Applicazioni conservando la copia precedente.

## Correzione nome visibile v3 — 18 settembre 2026

- [x] Verificati nome del bundle e processo aperto: già LIMEN Vault v3. Rilevata intestazione interna ancora LIMEN VAULT e vecchie copie in Applicazioni.
- [x] Uniformati intestazione, benvenuto, titolo HTML e titolo guida a LIMEN Vault v3.
- [ ] Verifiche, build, firma/notarizzazione aggiornata e sostituzione della consegna v3 conservando il pacchetto precedente.
- [x] Verifica UI nel candidato firmato: intestazione, benvenuto, titolo HTML e guida riportano v3.
- [x] App corretta installata in /Applications/LIMEN Vault v3.app e aggiornata in USER INSTALL. Firma/Gatekeeper PASS; tutti i file identici al candidato verificato nella UI. Copia precedente conservata in STORICO/2026-09-18-v3-prima-correzione-nome.
- [ ] Consegna finale aggiornata anche con il pulsante lime richiesto durante il lavoro; candidato con solo nome corretto superato.

## Consegna v3 — 18 settembre 2026

Richiesta autorizzata: aggiornare HELP e guide, compilare, firmare/notarizzare e consegnare v3 in USER INSTALL. Nessun commit/push richiesto. Consegne precedenti preservate.

- [x] Verificata documentazione contro i sorgenti; aggiornati flusso automatico, storage, API, limiti, retrieval e percorso manuale facoltativo.
- [x] HELP integrato e documenti offline inclusi nel bundle v3 (0.3.0).
- [x] Preparato OCR come componente del bundle da firmare e notarizzare insieme all’app.
- [x] Verifiche v3: pnpm test/typecheck/build PASS in sequenza; Rust 50 lib + 18 main PASS; collegamenti interni guida e HELP validi.
- [x] Build Tauri v3 PASS; HELP senza Vault e ricerca «wiki» verificati nell’app firmata. Guide incorporate confrontate byte per byte. OCR firmato verificato su PDF, scansione PDF e PNG.
- [x] App firmata Developer ID, Apple Accepted (966ae51a-9f81-4527-9335-1dbfcfb4d48e), ticket e Gatekeeper PASS.
- [x] DMG firmato, Apple Accepted (6a8ec9bb-2ef7-4006-b7e8-5ecf372727f1), ticket e Gatekeeper PASS.
- [x] Consegnati in USER INSTALL: LIMEN Vault v3.app, LIMEN-Vault-v3-arm64.dmg, HELP v3, guida estesa, manuale TXT, LEGGIMI e SHA256SUMS-v3. Avvio dal DMG e dalla copia consegnata verificato; HELP aperto senza Vault. Pacchetti precedenti invariati.
- [x] API reale dalla v3 firmata: 7/7 originali elaborati, 4 wiki generate; UI ricerca e anteprima RAG includono note/wiki automatiche senza inclusione bozze. Originali e marcatori verificati.
- [ ] Batch completo: una quinta wiki fermata dopo tre tentativi, ultimo errore HTTP 429. Precedente citazione invalida rifiutata. Nessun quarto tentativo; Vault di test in pausa.


## Automazione RAW → RAG + wiki LLM — 17 settembre 2026

Stato: implementazione e verifiche locali completate; collaudo API reale eseguito nella v3 firmata con residuo HTTP 429 su una wiki. Milestone automazione aperta per il residuo API; app/DMG v3 consegnati e notarizzati come indicato sopra. Piano: `IMPL_PLANS/IMP_PLAN_AUTO_KNOWLEDGE.MD`.

- [x] Verificata architettura attuale: compilazione manuale RAW → proposta, ricerca lessicale e selezione AI delle fonti approvate.
- [x] Implementata coda persistente automatica: rilevazione nuovi file/modifiche, deduplica, ripresa e tentativi limitati.
- [x] Implementate conversione locale/OCR per PDF, Office, fogli, immagini e testi, normalizzazione conservativa, classificazione AI e archiviazione nelle categorie; originali preservati.
- [x] Implementate wiki tematiche con provenienza, aggiornamento delle dipendenze e protezione delle modifiche umane.
- [x] Implementate indicizzazione automatica e retrieval di fonti elaborate/wiki senza approvazione umana per documento, con distinzione dalle fonti approvate manualmente.
- [x] Implementate configurazione iniziale, importazione e stato/eccezioni nell’interfaccia italiana.
- [x] Test locali del flusso e della conversione: provider simulato, PDF/PDF scansionato/PNG/Word/PPTX e foglio con 1201 righe. Esiti in AUTO_KNOWLEDGE_EVIDENCE.
- [x] Suite finali: pnpm test → typecheck → build PASS; Rust 50 lib + 18 main PASS. Primo pnpm test fermato da riferimento testuale al bucket nella guida preesistente; dicitura UI semplificata, senza indebolire il test.
- [x] UI nativa: caricamento di DOCX sintetico dal selettore macOS, messaggio «1 file caricati.», copia RAW byte per byte identica. Vault temporaneo isolato, automazione disattivata per questa prova.
- [ ] API reale completa: 7/7 fonti e 4 wiki elaborate dalla v3 firmata; una wiki ferma dopo tre tentativi (ultimo HTTP 429). Conversione generativa scartata e sostituita da estrattori nativi.
- [x] Build Tauri finale PASS: bundle locale LIMEN Vault.app generato.
- [x] Distribuzione v3 completata nella cartella USER INSTALL; batch API non integralmente completato, residuo tracciato sopra.
...
2. Priorità attuale: concludere il collaudo API reale dell’automazione RAW dopo l’autorizzazione Portachiavi a vault-check. La precedente discrepanza cron è superata dagli esiti del 13 settembre registrati sopra; M10 resta aperta per i collaudi residui.
...
## TODO LIST precedente

MEMORIA AZIENDALE — 18 settembre 2026
- Dipendenza di rilascio: completare il flusso unico e il collaudo di consultazione/ricerca prima della consegna combinata. Audit e criteri in IMPLEMENTATION/2026-09-18_UX_MEMORIA_AZIENDALE.md; non è una nuova app già pronta.
- La qualità semantica richiede un corpus rappresentativo con domande/passaggi attesi; nessuna misura di affidabilità o prestazione inventata. L’indice reale oggi contiene soprattutto proposte manuali.
- Corretta diagnosi Conoscenza: una nota in Casi studio, Clienti vuota; non assenza totale delle note.

USO REALE — Vault /Users/cesare/Documents/VAULT: 56 RAW presenti; automazione mai configurata. Selezione modello e attivazione iniziale necessarie per classificazione/wiki. Correzioni contatore, modelli e ricerca in corso; candidati nome/lime superati dalla build combinata.

PULSANTE LIME — modifica richiesta durante la correzione nome; nuova build/notarizzazione comprende entrambe. Nessun nuovo blocco.

CORREZIONE NOME V3 — rigenerazione pacchetto in corso; nessun nuovo accesso necessario.

CONSEGNA V3 — 18 settembre 2026
- Profilo notarizzazione recuperato dalla precedente configurazione e verificato con Apple: LIMEN-M9. Nessuna informazione ulteriore richiesta.
- Blocco Portachiavi del precedente harness superato per la v3 firmata: 7/7 originali e 4 wiki elaborati. Una wiki ferma a tre tentativi (ultimo HTTP 429); nessuna riprova finché non è risolta la causa API. Vault sintetico in pausa.
- Consegna v3 completata in USER INSTALL: app/DMG firmati, Apple Accepted, ticket e Gatekeeper PASS; HELP/guide aggiornati. Nessun blocco di distribuzione.

AUTOMAZIONE RAW — dipendenze 17 settembre 2026
- Classificazione e wiki richiedono modello OpenAI e chiave configurati; offline conservare la coda senza inventare risultati.
- Estrattori nativi/OCR e accesso API dalla v3 firmata collaudati; residuo reale: una wiki HTTP 429, dettagli V3_EVIDENCE/live-ui-api.json.
- Test e build Tauri PASS; v3 firmata/notarizzata consegnata con guide. Le app precedenti sono preservate e non includono il nuovo flusso.

AUDIT 14 SETTEMBRE: usare la coppia pii-crm.vercel.app → m3mai-backend.onrender.com per la consegna produzione. CRM .env.local punta invece a staging; non cambiare configurazione implicitamente. Riferimento: CLAUDE AUDIT - LIMEN CRM MEMAI.md. Stato Git del 13 superato per LIMEN: remoto main verificato oggi 8c8d6a9; MEMAI branch remoto c7c47ee.
STATO OPERATIVO — 13 settembre 2026
PUBBLICATO E COLLAUDATO — salute MEMAI ok; Workbench Codex read attivo; ricerca reale completata con risposta italiana e 8 fonti, annullamento durante esecuzione PASS. Backend 9043ff5, Render dep-dajgdl0jo6nc73dp4lr0 Live. CRM dpl_CSZPgsq6uTaMspWnTZJFVmAtpg6R Ready su pii-crm.vercel.app. Pre-deploy: entrambi gli script PASS; schema u1c2d3e4f5a6 = head. Commit/push MEMAI autorizzati ed eseguiti su codex/research-runtime; nessun merge main.
VERIFICHE: backend 436 passed + 49 subtests (23 skipped, 2 deselected); CRM 12/12 test, typecheck/build PASS; lint zero errori, una direttiva eslint superflua segnalata.
RESIDUI DIPENDENTI DA DATI/DISPOSITIVI:
- Note reali e mapping: Vault /Users/cesare/Documents/VAULT contiene solo HOME.md e VAULT_RULES.md di sistema. Domanda inviata su documento approvato e Cliente/Progetto esatti. Nessuna nota fittizia o di sistema pubblicata.
- Secondo Mac non disponibile. M10 offline in account macOS pulito non certificata. Non dichiarare M10 integralmente chiusa.
- M9 e DMG italiani notarizzati in USER INSTALL preservati. Nessun recupero manuale ingestion, nessuna variazione globale storage CRM.
OPERATIVITÀ: servizio Render ora segue codex/research-runtime, auto-deploy Off; main non modificato. CRM modifiche pubblicate via CLI, checkout locale conservato senza commit/push.
EVIDENZE: progetto LIMEN, IMPLEMENTATION/2026-09-13_RESEARCH_RELEASE.md. Storico salute/flag: IMPLEMENTATION/2026-09-13_MEMAI_ATTIVAZIONE.md.
