...
- **Verifica Compilazione Commit Intermedi**:
  - fase5i-check-commit-n1.log a fase5i-check-commit-n6.log (tutti 0 errori).
- **Patch Cumulativa**:
  - IMPLEMENTATION/WINDOWS_BUILD_EVIDENCE/fase-5i.patch generata con git diff 6b04271 57071f8.

---

## 5. FASE 5i-bis — Risoluzione Correzioni D1–D5 e Verifica Auditor

- **Data / Ora:** 2026-09-24 18:25 (UTC+8)
- **Stato:** **D1, D2, D4, Risposta N7 CHIUSI DALL'AUDITOR; D3 e D5 APERTI (in transizione verso FASE 5i-ter)**.
- **Base Commit FASE 5i:** `66d9bf2`.
- **Ultimo Commit di Codice 5i-bis:** `d8ddd86` (fix D4).
...
- **Rapporti di Consegna:** 
  - `IMPLEMENTATION/WINDOWS_BUILD_EVIDENCE/fase-5i-bis-rapporto.md` (rettificato P1, P2, P4)
  - `IMPLEMENTATION/WINDOWS_BUILD_EVIDENCE/fase-5i-rapporto.md` (ripulito D5, commit N6 corretto a `57071f8`, patch 8 file, falsa dichiarazione 0 scritture rettificata)

### Esito Verifica Auditor su FASE 5i-bis:
1. **CHIUSI**:
   - **D1**: Ramo non proprietario senza lock durante `check_health`/`status` e senza seconda acquisizione (`8de998b`).
   - **D2**: `get_binary_path` eseguito fuori mutex; funzione condivisa `log_panel_open_status` (`b4ab068`).
   - **D4**: Ripristinato PID del servizio in stato "in avvio" con relativo test unitario (`d8ddd86`).
   - **Risposta N7**: Analisi e tracciamento completo dell'albero di chiamata di `process_pending_extractions` e `save_catalog`.
2. **APERTI (FASE 5i-ter)**:
   - **D3 (Isolamento Modelli e Log)**:
     - Rettificato l'esito: tra le 17:45:54 e le 17:47:41 UTC+8, le corse parziali di `cargo test` durante lo sviluppo di 5i-bis hanno scritto 5 nuovi log in `C:\Users\user\LIMEN Vault\models\logs\` e troncato `llama-server.log` a 0 byte (modificato alle 17:46:59), cancellando 5 log dalla rotazione.
     - L'identità prima/dopo ha dimostrato che solo la corsa finale non ha scritto.
     - L'eliminazione dei rami `#[cfg(test)]` nelle funzioni di produzione, l'adozione sistematica di mock binary per i test e cartelle isolate per test sono pianificate per la FASE 5i-ter (T1-T4).
   - **D5 (Rapporto Pulito e Precisione Asserzioni)**:
     - Rigenerato `fase-5i-rapporto.md` privo di caratteri di controllo corrotti a partire da `e8351bf`.
     - Corretto riferimento commit N6 a `57071f8`.
     - Corretto numero file nella patch 5i a 8 file.
     - Dichiarata falsa la frase "0 scritture nei modelli condivisi" per la FASE 5i.
     - Rimossi claims non supportati da log/codice in `fase-5i-bis-rapporto.md`.
     - Documentate esplicitamente le modifiche al comportamento runtime dell'app rilasciata (`get_models_dir` con `LIMEN_MODELS_DIR`, check servizio sano antecedente a modello, check binario antecedente a modello).

### Verifiche di Bisezione e Test Suite FASE 5i-bis:
