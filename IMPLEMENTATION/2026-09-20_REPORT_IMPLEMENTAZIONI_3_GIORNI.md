...
- **Ultimo commit descritto:** `a845ad0` (codice di prodotto: C12); i commit successivi contengono solo evidenze e questo report
...
| 20/09 21:0x | `04db6a6` | Pulizia dei `llama-server` orfani (pidfile + terminazione all'avvio); badge del servizio riletto all'inizio del ricalcolo |
...
- **Ciclo di vita**: avvio su richiesta (non all'apertura dell'app); porta scelta libera con `TcpListener::bind("127.0.0.1:0")` (`find_free_port`, riga 304); argomenti `-m <modello> --embedding --host 127.0.0.1 --port <p> -ub 2048 -b 2048`; controllo di salute su `/health` entro 25 s; verifica che il vettore abbia esattamente 1024 componenti; istanza unica; spegnimento agganciato a `RunEvent::ExitRequested`/`Exit` in `main.rs`; massimo 3 riavvii dopo caduta. **Orfani** (dal commit `04db6a6`): il pid del figlio è registrato in `models/llama-server.pid`; all'avvio dell'app e prima di ogni avvio del servizio, un server registrato che risulti orfano (genitore `launchd`, riga di comando `llama-server … --embedding`) viene terminato; un server con genitore vivo o un processo diverso con lo stesso pid non viene toccato. Collaudato con `kill -9` dell'app (§6.3).
...
Passando da OpenAI (1536) a Locale (1024) tutti i vettori vanno ricalcolati. Il ricalcolo scrive in `00_SYSTEM/EMBEDDINGS_CACHE.staging.json` e la cache precedente resta intatta fino al completamento; l'operazione è ripristinabile senza ripartire da zero. Il valore «846,64 s per 4.633 passaggi» riportato in una versione precedente di questo report **non era una misura**: era una costante cablata nel banco di prova di AG, rimossa in `99d8fd1`. Misura reale dall'app installata (§6.3): **9.458 passaggi in 49 min 01 s = 0,31 s/passaggio** su Mac M2 8 GB con Metal (testo contestualizzato, staging riscritto ogni 320 passaggi, memoria libera 26 %); nel banco a lotti con testo grezzo 0,09 s/passaggio con Metal e 0,65 s su sola CPU. Richieste al server locale con timeout 300 s e lotti da 16 passaggi (`611e943`); l'evento `embeddings_sync_progress` (elaborati, totale, percentuale) è emesso all'avvio e dopo ogni lotto. **Misura strumentata del 21/09** (`IMPLEMENTATION/V3_AUDIT_CLOSURE_EVIDENCE/MISURE_RICALCOLO/`): il 98,8 % del tempo è nel calcolo del server, lo staging pesa meno dell'1 %, i lotti da 64 sono più lenti (+20 %) dei lotti da 16, e la velocità varia con il carico del Mac (0,12–0,20 s/passaggio nella stessa corsa). Nessuna ottimizzazione del codice è giustificata dalle misure. `needsReindex` è calcolato confrontando gli identificativi dei passaggi del catalogo con le chiavi della cache (non solo la dimensione).
...
- **C12** (21/09) bonus «frase esatta nel passaggio» con `contains`: un termine breve prendeva 15 punti anche come sottostringa di altre parole («personal» in «personale», «lead» in «leader») e, valendo solo per documenti con BM25 zero, i documenti con la sola sottostringa superavano quelli con il termine vero (50 pari merito a 15,0 su «personal») → confronto a parola intera; termini: lessicale P@1 0,470 → 0,576, ibrido 0,485 → 0,636, R@10 ibrido 0,955 → 1,000; frasi invariate.
- **Ricalcolo cache dall'interfaccia** (quattro difetti scoperti nel collaudo del §6.3, corretti in `611e943`; più due residui — badge del servizio e `llama-server` orfano dopo `kill -9` — corretti in `04db6a6`): (a) GPU Metal mai caricata su Mac con Homebrew ggml (solo CPU, 7× più lento); (b) timeout HTTP 60 s con lotti da 64 → richiesta annullata a metà (nel log del server «cancel task» esattamente 60 s dopo la richiesta); (c) evento `embeddings_sync_progress` mai emesso dal backend → barra ferma a 0,0 %; (d) con cache assente il report mostrava «0 passaggi (1536 dim)» e un finto «disallineamento» (default della struct). Prove in `COLLAUDO_COMPITO_3_RICALCOLO_CACHE/`.

### 5.4 Aperti
- **A04**: sul corpus reale lessicale R@10 1,000 / P@1 0,939 e ibrido R@10 1,000 / P@1 0,927 (82 frasi esatte). **Termini** (21/09, 66 casi etichettati dall'utente, vault da 114 documenti, dopo C12): ibrido P@1 0,636 / R@10 **1,000**, lessicale P@1 0,576 / R@10 0,955; dettaglio in `A04_TERMINI_V2_C12/RUN_NOTE.md`. Il requisito «100 % nella vista pertinente» è soddisfatto in ibrido anche sui termini; resta senza set di prova il tipo *codice* (nel corpus reale i codici sono rari).
...
Osservazione (per costruzione, non un difetto): l'indice lessicale costruito dall'app **memorizza** passaggi solo per i file di `20_RAW_SOURCES` (`search.rs`, `index_vault_search`); per le note delle altre cartelle memorizza il testo completo (`content_preview` = corpo intero, verificato: fino a 225.174 caratteri per le proposte) e **al momento della ricerca** lo spezza in passaggi e sceglie il localizzatore migliore (`search.rs`, `dynamic_passages`). Quindi anche le note fuori da `20_RAW_SOURCES` ricevono localizzatore ed estratto dal canale lessicale; la differenza è solo di costo (spezzettamento ripetuto a ogni ricerca) e di punteggio BM25 (calcolato sul documento intero). Una prima versione di questa nota, e la proposta «opzione B» discussa il 21/09, davano per assente il localizzatore lessicale di quelle note: era un errore di lettura, corretto qui. **21/09, 09:31**: su richiesta dell'utente l'indice del Vault reale è stato ricostruito da zero con il codice `a845ad0` (`vault-check search-index`, 2,5 s): 114 documenti, passaggi memorizzati per tutte le cartelle (20_RAW_SOURCES 4.633, 90_PROPOSALS 4.725, 07_CASE_STUDIES 75), 69.451.660 byte; l'indice precedente riusava voci del 18/09 senza passaggi memorizzati (`search.rs:497`, riuso per hash e data invariati).
...
- Esiti del 20/09: app `07e5abcb-aaee-41fa-9adb-f056209088a0` **Accepted**; DMG `56892416-9783-4033-9fcd-3fd510c65bf9` **Accepted**; verifica indipendente sull'app installata: `spctl` accepted / `source=Notarized Developer ID`, `stapler validate` OK, `codesign --verify --deep --strict` valido. Secondo giro per `a08e769`: app `6b0a0583-4974-4f55-bf96-278c2921c475` **Accepted**, DMG accettato e graffettato, installata in `/Applications` e verificata (`spctl` accepted, `stapler validate` OK, firma profonda valida) alle 18:5x UTC+8. È la build usata per i collaudi §6.1–6.2 e per la prima corsa del §6.3. **Terzo giro per `611e943`** (18:39–18:45 UTC+8): app `99caeb1e-ac3a-455e-8e9a-af71ef322fcf` **Accepted**, graffettata (`stapler validate` OK, `spctl` accepted / Notarized Developer ID); DMG `f6d8b2f8-3278-4f00-872e-70ca58f97df9` **Accepted**, graffettato e accettato da Gatekeeper; `LIMEN-Vault-v3-arm64.dmg` SHA-256 `3f7a333d8754d2a99a250412cdb1f5b715ae8ca5c45441584d09110674afd0e8` in `.local/limen-v3-release/` (la release precedente è conservata in `.local/limen-v3-release-a08e769/`). **Quarto giro per `04db6a6`** (21:13–21:16): app `8cf464f0-29a3-4867-ab6b-cf3b0d00d92d` **Accepted** e graffettata; DMG `a59789e3-d911-4c4a-b946-255880e5c452` **Accepted**, graffettato, Gatekeeper accepted; `LIMEN-Vault-v3-arm64.dmg` SHA-256 `55f78cd8c03b6aee76803a34bb509e93d2497a2ffcee585e68c774a613eff447` in `.local/limen-v3-release/` (release precedenti conservate in `.local/limen-v3-release-a08e769/` e `-611e943/`). **Copia graffettata installata in `/Applications`** e verificata (firma profonda, `stapler validate`, `spctl` accepted / Notarized Developer ID). **Quinto giro per `4740e77`** (manuali corretti, 22:21–22:24): app `d2f0c233-d963-4045-9025-cb17bef91370` **Accepted** e graffettata; DMG `644b5fe4-8a33-48b3-8352-3f9d4b75b415` **Accepted**, graffettato, Gatekeeper accepted. **Consegna: `USER INSTALL/LIMEN-Vault-V4-arm64.dmg`** (SHA-256 `f64b718ed20b9fff01b66c753163bf438437335ae73e4cd0e250d40b4cc9dfeb`, 50.301.214 byte), stesso file di `.local/limen-v3-release/LIMEN-Vault-v3-arm64.dmg`; contenuto verificato montandolo (app, HELP v3, guida, manuale in testo e LEGGIMI con le correzioni). Copia graffettata installata in `/Applications` e verificata.
**Sesto giro per `a845ad0` (C12)** (21/09, 08:3x): app `f03d62a1-fd88-44b7-a8dc-b6b7d179d868` **Accepted** e graffettata; DMG `190c3440-67b4-4dad-8382-6261124e9e93` **Accepted**, graffettato, Gatekeeper accepted. `USER INSTALL/LIMEN-Vault-V4-arm64.dmg` **sostituito** (SHA-256 `11fa8107658326ce26c84c6944849de2389c5fe474ccb4021a94ba45779625ef`); app graffettata installata in `/Applications` e verificata. **È la build finale.**
